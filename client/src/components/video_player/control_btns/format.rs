use enums::IconInput;
use invidious::{find_audio_format, DashFormat, Format, Formats};
use leptos::*;
use utils::is_webkit;

use crate::{components::icon::Icon, contexts::{config::PlayerConfigCtx, PlayerState}};
...
pub fn FormatList() -> impl IntoView {
    ...
    let player_config = expect_context::<PlayerConfigCtx>();
        ...
        formats
            ...
            .map(|format| {
                ...
                let audio_format = current_format.get().map_or(
                    find_audio_format(
                        &formats.get(),
                        player_config.config.get().default_audio_quality,
                        is_webkit(),
                    )
                    .ok(),
